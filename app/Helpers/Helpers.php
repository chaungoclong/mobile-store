...
class Helpers
{
    ...
    {
        $output = '';
        if ($promotion_price != null && $promotion_start_date <= date('Y-m-d') && $promotion_end_date >= date(
                'Y-m-d'
            )) {
            $output = '<div class="promotion-percent">Giảm ' . round(
                    100 * ($price - $promotion_price) / $price
                ) . '%</div>';
        }
        return $output;
    }
    ...
    public static function get_real_price($price, $promotion_price, $promotion_start_date, $promotion_end_date)
    {
        ...
        if ($promotion_price != null && $promotion_start_date <= date('Y-m-d') && $promotion_end_date >= date(
            ...
            $output = '<strong>' . number_format($promotion_price, 0, ',', '.') . '₫</strong><del>' . number_format(
                    $price,
                    0,
                    ',',
                    '.'
    ...
    public static function formatVietnameseCurrency($amount): null|string
    {
